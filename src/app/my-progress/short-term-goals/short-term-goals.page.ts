...
@Component({
  ...
  templateUrl: './short-term-goals.page.html',
  styleUrls: ['./short-term-goals.page.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ShortTermGoalsPage implements OnInit {
  ...
  constructor(private translate: TranslateService, private change: ChangeDetectorRef, private authService: AuthService, private progressService: ProgressService, private modalController: ModalController, private alertController: AlertController) {
  ...
  getObjectives(uid: string) {
    ...
    let ld = this.sumarDias(date, 30);
    this.items = this.progressService.getObjectives(uid);
    this.items.subscribe(res => {
      this.goals = [];
      res.forEach(e => {
        if(e.fecha_cumplimiento >= fd && e.fecha_cumplimiento <= ld){
          this.goals.push(e);
        }
      })
